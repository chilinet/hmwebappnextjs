...
/** UUID-Vergleich ohne Berücksichtigung von Großschreibung und Bindestriche */
export function normAssetId(id) {
  if (id == null || id === '') return '';
  return String(id).replace(/-/g, '').toLowerCase();
}

/** Asset-ID aus rohem Strukturknoten (ThingsBoard { id, entityType } oder String). */
export function resolveRawStructureNodeAssetId(node) {
  if (!node) return '';
  let raw = node.id;
...
/** Oberster sinnvoller Einstiegsknoten: erste Wurzel mit ID, sonst erster Nachfahre. */
export function getTopRootAssetIdFromStructureTree(tree) {
  if (!tree?.length) return '';
  function firstIdDeep(nodes) {
    for (const n of nodes || []) {
      const id = resolveRawStructureNodeAssetId(n);
      if (id) return id;
      if (n.children?.length) {
        const sub = firstIdDeep(n.children);
        if (sub) return sub;
      }
    }
    return '';
  }
  return firstIdDeep(tree);
}

/** Sucht in der von convertToTreeViewFormat gelieferten flachen Liste */
export function findFlatTreeNodeByAssetId(flatNodes, assetId) {
  if (!flatNodes?.length || assetId == null || String(assetId).trim() === '') return null;
  const t = normAssetId(assetId);
  return flatNodes.find((n) => n.id && normAssetId(n.id) === t) || null;
}

/**
 * Nur der gewählte Einstiegsknoten und alle Nachfahren; alle Vorfahren werden ausgeblendet.
 * Gibt die unveränderte Liste zurück, wenn der Knoten nicht gefunden wird.
 */
export function extractSubtreeRootedAtAssetId(nodes, assetId) {
  if (!nodes?.length || assetId == null || String(assetId).trim() === '') {
    return nodes || [];
  }
  const target = normAssetId(assetId);

  function findNodeDeep(list) {
    for (const node of list) {
      if (node.id && normAssetId(node.id) === target) return node;
      if (node.children?.length) {
        const f = findNodeDeep(node.children);
        if (f) return f;
      }
    }
    return null;
  ...
  function cloneWithSubtree(node) {
    return {
      ...node,
      children: Array.isArray(node.children) ? node.children.map(cloneWithSubtree) : []
    };
  }

  const found = findNodeDeep(nodes);
  if (!found) return nodes;
  return [cloneWithSubtree(found)];
}
import {
  faMapMarkerAlt,
  faBuilding,
...
export function getPathToNode(nodeId, nodes) {
  const target = normAssetId(nodeId);
  function findPath(nodeList, currentPath = []) {
    for (const node of nodeList) {
      const newPath = [...currentPath, node.id];
      if (node.id && normAssetId(node.id) === target) return newPath;
...
export function getAllSubordinateNodeIds(nodeId, nodes) {
  function findNode(nodeList, targetId) {
    const t = normAssetId(targetId);
    for (const node of nodeList) {
      if (node.id && normAssetId(node.id) === t) return node;
...
export function getAllSubordinateNodes(nodeId, nodes) {
  ...
  function getNodePath(nid, nodeList, path = []) {
    const t = normAssetId(nid);
    for (const node of nodeList) {
      const currentPath = [...path, node];
      if (node.id && normAssetId(node.id) === t) return currentPath;
      if (node.children) {

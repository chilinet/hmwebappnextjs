...
export async function getPgConnection() {
  try {
    if (!pool) {
      ...
      debugLog('PostgreSQL connection pool created');
    }
    ...
    try {
      ...
      if (error.code === 'ECONNCLOSED' || error.code === 'ECONNRESET') {
        debugLog('PostgreSQL connection lost, recreating pool...');
        pool = null;
        pool = new Pool(config);
        debugLog('PostgreSQL connection pool recreated');
      } else {
        throw error;
      }
    }
...
// Graceful shutdown (einmal registrieren — vermeidet MaxListenersExceeded bei HMR)
const g = globalThis;
if (!g.__hmPgPoolShutdownRegistered) {
  g.__hmPgPoolShutdownRegistered = true;
